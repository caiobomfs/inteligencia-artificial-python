...
def teste_generalizacao(sinapses):
    ...
    for x in range(359):
        ...
        seno[x] = (math.sin(x*math.pi/180)) * (math.cos(x*math.pi/180)*1.3)
        coseno[x] = (math.cos(x*math.pi/180)) * (math.sin(x*math.pi/180)*1.2)
        coeficiente[x] = (x * math.pi/180)*0.8
        entradas[x] = [termo_proporcionalidade, seno[x], coseno[x], coeficiente[x]]
        saida_parcial[x] = saida_adaline(pesos_sinapses, entradas[x])
...
ax.set(xlabel='angulo(º)', ylabel='funcoes', title="neuronio Adaline")
ax.grid()
